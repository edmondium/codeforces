...
template<typename T>
inline T readInt() {
    T x = 0; bool neg = false; char c = getchar();
...
inline void readInt(T& x, Ts&... xs) {
    x = readInt<T>(); (readInt(xs), ...);
}

template<typename T>
inline void writeInt(T x) {
    if (x < 0) { putchar('-'); writeInt(-x); }
    else { if (x >= 10) writeInt(x / 10); putchar(char('0' + x % 10)); }
}
...
int main() {
    ...
    for (int j = 1; j <= m; ++j) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            while (!dq.empty() && b[dq.back()][j] >= b[i][j]) dq.pop_back();
            dq.push_back(i);
            if (i >= h && j >= w) {
                ll sum = s[i][j] - s[i-h][j] - s[i][j-w] + s[i-h][j-w];
                ll mn = b[dq.front()][j];
